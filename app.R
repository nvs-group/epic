...
ui <- dashboardPagePlus(
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      menuItem("Go Exploring", tabName = "buildbasic", icon = icon("tasks")),
...
server <- function(input, output, session) {
  ...
  observe ( {  
    ...
    output$nvs.choice.table <- renderDataTable({
      DT::datatable(data = table_var()  %>% select(input$column.names), 
                    options = list(pageLength = input$RecordsNum, filter = TRUE),selection = list(mode = "multiple"))
    })
  })
  ...
  observeEvent(input$pre.income, {
    updateSliderInput(session, "nvs.income", "Min starting salary:", value = input$pre.income)
  })
  # from income level choice on preference page  
  ...
  observeEvent(input$pre.tuition, {
    updateSliderInput(session, "nvs.tuition", "Max yrly Ed costs:", value = input$pre.tuition)
  })
