---
title: "Data Persistence using SQL in EPIC"
author: "John Chapman"
date: "11/11/2019"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

Overview for data persistence goal:
1. Select dev server
2. Install sqlite (mysql?)
3. Transfer data to sql
  + transfer Master1 
  + cip1
  + soc1
4. Alter any code?

.. Replicate master branch using sql input/output


# Select Dev Server

* WelcomeU - to separate production from dev.  But, would mean double the maintenance/upgrades. But prob better development vs production app management.
+ What needs to happen to make Welcome U the dev server?
++ R installed - check
++ Shiny installed - check
++ git installed - check
++ install ssh public key to login from ipad

# Install Sqlite or MySQL
* SQLite is probably a sufficient sql engine for the number of users we expect and to avoid the upkeep of MySQL
+ sudo su - -c "R -e \"install.packages('RSQLite', repos='http://cran.rstudio.com/')\""

# Transfer data to sql
* copied epicdb.sqlite file from Dropbox/R-Studio/EPIC-old/data

## 2019 11 11 - first run app() results:
> Listening on http://127.0.0.1:4886  
> Warning: Error in %in%: object 'school.name' not found

Need to check on school.name field, did it transfer ok?  Is it newer than the epicdb.sqlite file? Do I need to create a new epicdb.sqlite database?  
Also need to create or tables for the soc and cip

2019 11 11 13:39 - re-imported the csv file from the zip file into sqlite, and the app runs great.
Had to use Epic username to get in
So, success in loading the master1 file
Let me move on to SOC, CIP - added these as separate tables within the database

Need to know what the user_data structure is to capture user preferences
### User data table
<!-- Sources -->
<!-- https://github.com/datastorm-open/shinymanager -->
<!-- https://github.com/paulc91/shinyauthr -->

acct_id: user id
user_name: login name
user_password: login password
acct_admin: "no" or "yes"
acct_email: email address
acct_created: date-time of creation

Successfully wrote to db using following code:

```{r}


 data_test <- data.frame(
   acct_first_last = "John Chapman",
   user_name = "Juancito",
   user_password = sapply("pass1",password_store),
   acct_email = "juancito@benitosburritos.com",
   acct_admin = "No",
   acct_created = as.character(Sys.time()),
  stringsAsFactors = FALSE
 )

sqlitePath <- "data/epic_20191111.sqlite"

db <- dbConnect(SQLite(), sqlitePath)
dbWriteTable(db,"accounts", data_test, append = TRUE)

# check results ----
dbListTables(db)
dbGetQuery(db, "SELECT * FROM accounts")

```

Next step: Try to insert user from the Create Account popup 


Wait a minute ... what about google authentication?  Isn't this better than creating our own authentication? In terms of updating passwords, hashing passwords, security, resetting?  
```{r}
library(googleAuthR)
# starts auth process with defaults
gar_auth()
#>The googleAuthR package is requesting access to your Google account. Select a 
#> pre-authorised account or enter '0' to obtain a new token. Press Esc/Ctrl + C to abort.

#> 1: mark@work.com
#> 2: home@home.com
```


Turns out there is a big problem with using a proxy server (nginx) while testing Google Authentication in RStudio server.  The proxy server changes the URL with a /p/236432/ addition to the URL.  I can't get Google to recognize the URL and so Google throws errors when I try to develop the code.  BUT, prileminary coding suggests this is definitel the way to go in a future version.  And, we can pull the user's email, and basic profile info (name?) directly from Google.  
So, I'm suggesting we don't do this for the initial build.


## sqlite authentication
I was able to build sql authentication into the development version of the app.
I commented out the dropbox credentialing code
I can
* create a new user using the "create Account" box
+ check if username already exsts, if yes, then error message, if not, then add to database
* log in with the new user
* found out
** Need to close the database in dbbrowser
** Need to dbdisconnect after every connect

OK, let me try to capture what I was able to do:
1. migrated the datapersistance_dev branch directly to nvsgroup.org server
* possible by cloning the epic repository into the folder "datapersistance_dev" inside the /srv/shiny-server/ folder (also created a new project in rstudio.nvsgroup.org by cloning the epic repository into a new folder with /R/datapersistance_dev)
* now I can continue to use the rstudio.nvsgroup.org server/website to develop and test (instead of a separate server), and I can actually run the app in a separate folder on the same server (back down to 1 server instead of 2)
2. Also, I was able to implement the check if a username exists code, and
3. sql authentication works (as far as I know)

Useful commands:
git clone git@github.com:nvs-group/epic.git datapersistance_dev [in the root of /svr/shiny-server/]
Also needed to copy the db to the data folder of the app manually


### sql scenario save, load
- created new scenarios table in epic.sqlite
* exported Epic.rds to csv (write.csv(Epic, "data/scenario.csv"))
* imported csv into epic.sqlite (via DB Browser for SQLite on Ernestine)
* for some reason I don't fully understand the import added a first column, I changed that to "acctid" INTEGER, NOT NULL, Primary Key

- insert scenario_temp dataframe into scenarios table
* stumped, error is not specific enough "Warning: Error in : near "?": syntax error"
* I'm using correct function (dbExecute)
* Must be incorrect data type?
